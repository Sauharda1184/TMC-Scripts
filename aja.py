    ...
    def Submit_button_command(self):
        ...
        try:
            ...
            if direction_number == 4:
                ...
                status_text.set(f"Camera {direction_number} returned invalid data format. Check IP address and date range. Error: {str(idx_err)}")
                status_message["text"] = status_text.get()
                nextstep = "no"
        ...
        while end_date != start_date and nextstep == "yes":
            ...
            try:
                ...
                while camera_number == 1 and nextstep == "yes":
                    ...
                    # Ensure we have exactly 12 columns - pad with None if needed
                    expected_cols = 12
                    if df1.shape[1] < expected_cols:
                        # Add missing columns filled with None
                        for i in range(df1.shape[1], expected_cols):
                            df1[i] = None
                    elif df1.shape[1] > expected_cols:
                        # Drop extra columns if somehow we got more
                        df1 = df1.iloc[:, :expected_cols]
                    ...
                    while camera_number < 3 and nextstep == "yes":
                        try:
                            ...
                            camera_number += 1
                                ...
                                # If less than 2 cameras are found, then raises an error.
                                status_text.set(f"Camera {camera_number} data parsing failed: {str(parse_err)}. Check data format.")
                                status_message["text"] = status_text.get()
                                nextstep = "no"
                                break
            ...
            try:
                while camera_number < 5 and nextstep == "yes":
                    ...
                    # Ensure we have exactly 12 columns - pad with None if needed
                    expected_cols = 12
                    if df2.shape[1] < expected_cols:
                        # Add missing columns filled with None
                        for i in range(df2.shape[1], expected_cols):
                            df2[i] = None
                    elif df2.shape[1] > expected_cols:
                        # Drop extra columns if somehow we got more
                        df2 = df2.iloc[:, :expected_cols]
