...
Uses direction bins only for the summary (zones 501–512: LT / Through / RT per sensor).
...
def synthetic_direction_rows_from_mqtt_payload(
    ...
    *,
    ...
    """
    One /iteris/count/ JSON object → synthetic rows usable by build_tmc_summary / build_sensor_summary.
    """
    rows: list[dict] = []
...
class App:
    ...
    def convert(self) -> None:
        ...
        if not summ:
            msg = f"No direction bins (501–512) for summary—wrote header only. Detail: {detail_p.name}."
        if diag:
