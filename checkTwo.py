...
class App:
    ...
    def Submit_button_command(self):
        ...
        def map_zone_to_direction(zone_name):
            # Only keep zones that already carry a standard direction prefix.
            # General/summary zones (e.g. "Zone") lack one and would double-count
            # vehicles already captured by the lane-specific zone detectors.
            if isinstance(zone_name, str) and zone_name.startswith(("EB", "WB", "NB", "SB")):
                return zone_name
            print(f"Skipping zone '{zone_name}': no EB/WB/NB/SB prefix.", file=sys.stderr)
            return None
            ...
            try:
                ...
                while camera_number == 1 and nextstep == "yes":
                    ...
                    for idx in range(len(df1)):
                        ...
                        df1.loc[idx, "Zone Name"] = map_zone_to_direction(original_zone)
            ...
            try:
                while camera_number < 5 and nextstep == "yes":
                    ...
                    for idx in range(len(df2)):
                        ...
                        df2.loc[idx, "Zone Name"] = map_zone_to_direction(original_zone)
