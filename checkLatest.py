...
class App:
    ...
    def Submit_button_command(self):
        ...
        def map_zone_to_direction(zone_name, camera_number):
            if not isinstance(zone_name, str):
                return None
            # Already has a standard direction prefix - keep as-is
            if zone_name.startswith(("EB", "WB", "NB", "SB")):
                return zone_name
            # Generic Zone detector — tag with direction prefix so we can decide later
            # whether to include it (for cameras with no named through zones)
            if zone_name == "Zone":
                if camera_number in custom_direction_mapping:
                    return custom_direction_mapping[camera_number] + "_Zone"
                return None
            # Descriptive lane name without direction prefix (e.g. "Left Turn Lane")
            # Prepend direction based on camera number using the configured mapping
            if camera_number in custom_direction_mapping:
                return custom_direction_mapping[camera_number] + zone_name
            print(f"Skipping zone '{zone_name}' on camera {camera_number}: no direction mapping found.", file=sys.stderr)
            return None
        ...
        while end_date != start_date and nextstep == "yes":
            ...
            try:
                ...
                while camera_number == 1 and nextstep == "yes":
                    ...
                    for idx in range(len(df1)):
                        ...
                        df1.loc[idx, "Zone Name"] = map_zone_to_direction(original_zone, camera_number)
                    df1 = df1[df1["Zone Name"].notna()].reset_index(drop=True)
            ...
            try:
                while camera_number < 5 and nextstep == "yes":
                    ...
                    for idx in range(len(df2)):
                        ...
                        df2.loc[idx, "Zone Name"] = map_zone_to_direction(original_zone, camera_number)
                    df2 = df2[df2["Zone Name"].notna()].reset_index(drop=True)
            ...
            if nextstep == 'yes':
                ...
                # --- Normalize descriptive lane zones and handle generic _Zone entries ---

                # Step 1: For zones named "XBLeft Turn Lane" or "XBRight Turn Lane",
                # the camera records all counts as ThroughCount instead of Left/RightTurnCount.
                # Override using Volume (total vehicles in lane) placed in the correct column.
                left_lane_mask = exiting["Zone Name"].str.contains("Left Turn", na=False)
                right_lane_mask = exiting["Zone Name"].str.contains("Right Turn", na=False)

                if left_lane_mask.any():
                    vol = pd.to_numeric(exiting.loc[left_lane_mask, "Volume"], errors="coerce").fillna(0).astype(int)
                    exiting.loc[left_lane_mask, "Left"] = vol
                    exiting.loc[left_lane_mask, "Through"] = 0
                    exiting.loc[left_lane_mask, "Right"] = 0

                if right_lane_mask.any():
                    vol = pd.to_numeric(exiting.loc[right_lane_mask, "Volume"], errors="coerce").fillna(0).astype(int)
                    exiting.loc[right_lane_mask, "Right"] = vol
                    exiting.loc[right_lane_mask, "Through"] = 0
                    exiting.loc[right_lane_mask, "Left"] = 0

                # Step 2: Decide which "_Zone" (generic detector) rows to keep.
                # If a direction already has a named through zone (e.g. EBT1, WBT1 — 3rd char is "T"),
                # the _Zone rows are redundant advance/summary detectors → drop them.
                # If no named through zone exists (e.g. WB only has "Left/Right Turn Lane"),
                # keep _Zone rows as the only source of through-traffic counts,
                # but zero out Left/Right to avoid double-counting with named turn-lane zones.
                for direction in ["EB", "WB", "NB", "SB"]:
                    named_mask = (
                        exiting["Zone Name"].str.startswith(direction, na=False) &
                        ~exiting["Zone Name"].str.endswith("_Zone", na=False)
                    )
                    has_named_through = any(
                        len(z) > 2 and z[2] == "T"
                        for z in exiting.loc[named_mask, "Zone Name"].dropna()
                    )
                    generic_mask = (
                        exiting["Zone Name"].str.startswith(direction, na=False) &
                        exiting["Zone Name"].str.endswith("_Zone", na=False)
                    ...
                    if generic_mask.any():
                        if has_named_through:
                            exiting = exiting[~generic_mask]
                        else:
                            exiting.loc[generic_mask, "Left"] = 0
                            exiting.loc[generic_mask, "Right"] = 0

                exiting = exiting.reset_index(drop=True)
                # ---------------------------------------------------------------
