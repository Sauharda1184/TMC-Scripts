...
Primary output (default): sensor summary layout:
  Time (HHMM), S1L/S1T/S1R .. S4L/S4T/S4R, Veh Total, plus a Total row.

Optional output: classic TMC layout:
  Time (HHMM), EBL..SBR, Veh Total, plus a Total row, using a Sensor→EB/WB/NB/SB mapping.

Summary uses automatic per-lane zones **701–740** (§10.1): per sensor, **701–702** = right
(R1+R2), **703–707** = through (T1–T5), **708–710** = left (L1–L3); columns **S?R/S?T/S?L**
sum those zone volumes. Also writes *_detail.csv (long tidy rows) for all zone types.

This file is self-contained (it does not import import_iteris_bindata.py).
"""
...
# Summary modes
SUMMARY_MODE_SENSOR = "sensor"
SUMMARY_MODE_CARDINAL = "cardinal"

# Classic direction order
...
# Classic TMC fieldnames (EBL..SBR)
SUMMARY_FIELDNAMES_CARDINAL = (["Time"] + [f"{c}{m}" for c in _CARDINALS for m in _LTR] + ["Veh Total"])

# Sensor fieldnames (S1L..S4R)
SUMMARY_FIELDNAMES_SENSOR = (["Time"] + [f"S{s}{m}" for s in _SENSORS for m in _LTR] + ["Veh Total"])

# Column order for detail CSV output (remaining keys appended alphabetically).
_CSV_KEY_ORDER = [
    "record_type",
    "source_file",
    "zone_number",
...
# Slot index within each 10-zone block (0..9): 0–1 R, 2–6 T, 7–9 L (see §10.1 table).
AUTO_MOVEMENT = {
    1: "R1",
    2: "R2",
    3: "T1",
...
def auto_zone_to_sensor_and_ltr(zone: int) -> tuple[int, str] | None:
    """
    Map automatic zone 701–740 to (sensor 1–4, summary suffix 'L'|'T'|'R').
    Per Iteris §10.1: slots 701–702 / 711–712 / … = right; 703–707 = through; 708–710 = left.
    """
    if not (701 <= zone <= 740):
        return None
    sensor = (zone - 701) // 10 + 1
    slot = (zone - 701) % 10
    if slot <= 1:
        return sensor, "R"
    if slot <= 6:
...
def parse_bindata_line(line: str) -> dict | None:
    ...
    if len(parts) < 11:
        return {"record_type": "malformed", "raw_line": raw, "field_count": len(parts)}
...
# --- MQTT helper utilities (used by import_iteris_mqtt.py) ---
...
def synthetic_direction_rows_from_mqtt_payload(
    ...
    *,
    ...
    """
    One /iteris/count/ JSON object → synthetic rows for build_tmc_summary / build_sensor_summary.
    Uses automatic zones **701–740** (Z701..Z740) per §10.1.
    """
    rows: list[dict] = []
    ...
    for zone in range(701, 741):
        ...
        if cat != "auto_lane":
            continue
        sensor = sensor_from_auto_zone(zone)
        if sensor is None:
            continue
        rows.append(
            {
...
def build_tmc_summary(rows: list[dict], sensor_to_cardinal: dict[int, str]) -> list[dict]:
    """Cardinal summary: EBL..SBR from auto zones 701–740 (aggregated L/T/R per sensor)."""
    ...
    for r in rows:
        ...
        try:
            ...
            continue
        mapped = auto_zone_to_sensor_and_ltr(zi)
        if mapped is None:
            continue
        sensor, suf = mapped
...
def build_sensor_summary(rows: list[dict]) -> list[dict]:
    """Sensor summary: S1L..S4R from auto zones 701–740 (aggregated L/T/R per sensor)."""
    acc: dict[int, dict[str, int]] = defaultdict(lambda: defaultdict(int))
...
def write_tmc_summary_csv(path: Path, summary_rows: list[dict]) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(SUMMARY_FIELDNAMES_CARDINAL), extrasaction="ignore")
        w.writeheader()
        for r in summary_rows:
            w.writerow({k: ("" if r.get(k) is None else r.get(k, "")) for k in SUMMARY_FIELDNAMES_CARDINAL})


def write_sensor_summary_csv(path: Path, summary_rows: list[dict]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=list(SUMMARY_FIELDNAMES_SENSOR), extrasaction="ignore")
        w.writeheader()
        for r in summary_rows:
            w.writerow({k: ("" if r.get(k) is None else r.get(k, "")) for k in SUMMARY_FIELDNAMES_SENSOR})


def write_summary_csv(path: Path, summary_rows: list[dict], mode: str) -> None:
    """Used by MQTT tool to write either summary shape."""
    if mode == SUMMARY_MODE_CARDINAL:
        write_tmc_summary_csv(path, summary_rows)
    else:
        write_sensor_summary_csv(path, summary_rows)
...
def write_outputs(
    ...
    out_base: Path,
    *,
    summary_mode: str = SUMMARY_MODE_SENSOR,
    sensor_to_cardinal: dict[int, str] | None = None,
) -> tuple[Path, Path | None, Path | None, list[dict]]:
    """
    Writes:
      {out_base}.csv — summary (sensor or cardinal)
      {out_base}_detail.csv — long tidy rows (all zone types)
      {out_base}_skipped_or_log.csv — optional diagnostics
    """
    if summary_mode == SUMMARY_MODE_CARDINAL:
        mapping = sensor_to_cardinal or {1: "EB", 2: "WB", 3: "NB", 4: "SB"}
        summary_rows = build_tmc_summary(rows, mapping)
        summary_path = out_base.with_suffix(".csv")
        write_tmc_summary_csv(summary_path, summary_rows)
    else:
        summary_rows = build_sensor_summary(rows)
        summary_path = out_base.with_suffix(".csv")
        write_sensor_summary_csv(summary_path, summary_rows)
...
class App:
    def __init__(self, root) -> None:
        ...
        self.sensor_to_cardinal: dict[int, str] = {1: "EB", 2: "WB", 3: "NB", 4: "SB"}
        self.summary_mode = tk.StringVar(value=SUMMARY_MODE_SENSOR)

        root.title("Iteris Vantage bin-data → CSV (sensor summary)")
        w, h = 520, 300
        ...
        tk.Label(root, font=ft, text="Output CSV base (main + _detail.csv):").place(x=12, y=68, height=24)
        ...
        tk.Label(
            ...
            font=ft,
            justify="left",
            text="Map Sensor 1–4 to EB/WB/NB/SB (used only for EBL…SBR mode).",
        ).place(x=108, y=128, width=392, height=32)

        tk.Radiobutton(
            root,
            font=ft,
            text="Summary by sensor (S1L…S4R)",
            variable=self.summary_mode,
            value=SUMMARY_MODE_SENSOR,
        ).place(x=12, y=160)
        tk.Radiobutton(
            root,
            font=ft,
            text="Summary by direction (EBL…SBR)",
            variable=self.summary_mode,
            value=SUMMARY_MODE_CARDINAL,
        ).place(x=250, y=160)

        tk.Button(root, font=ft, text="Convert", command=self.convert).place(x=12, y=192, width=100, height=32)
        tk.Label(root, font=ft, textvariable=self.status, anchor="w", relief="sunken").place(
            x=12, y=234, width=488, height=28
        )
    ...
    def show_direction_dialog(self) -> None:
        ...
        tk = self._tk
        d = tk.Toplevel()
        d.title("Sensor → approach (EB/WB/NB/SB)")
        w, h = 320, 220
        ...
        def save() -> None:
            ...
            for i in range(1, 5):
                ...
                if v not in choices:
                    self._messagebox.showerror("Invalid", f"Sensor {i} must be one of: {', '.join(choices)}")
                    return
                m[i] = v
    ...
    def convert(self) -> None:
        ...
        out_base = Path(out) if out else src.with_name(src.stem + "_clean")
        ...
        summary_p, detail_p, diag, summ = write_outputs(
            ...
            out_base,
            summary_mode=mode,
            sensor_to_cardinal=self.sensor_to_cardinal,
        )

        n_bin = sum(1 for r in rows if r.get("record_type") == "bin")
        n_intervals = max(0, len(summ) - 1) if summ else 0
        msg = f"Summary: {summary_p.name} ({n_intervals} time bins + Total). Detail: {detail_p.name} ({n_bin} bin rows)."
        if not summ:
            msg = (
                f"No automatic lane bins (701–740) for summary—wrote header only. "
                f"Detail: {detail_p.name}."
            )
        if diag:
...
def main_cli(argv: list[str]) -> int:
    ap = argparse.ArgumentParser(description="Iteris bindata .txt → clean summary CSV.")
    ap.add_argument("bindata_txt", nargs="?", help="Path to Iteris bindata .txt file")
    ap.add_argument("output_base", nargs="?", help="Output base path (without .csv); default: <stem>_clean")
    ap.add_argument(
        "--summary-mode",
        choices=[SUMMARY_MODE_SENSOR, SUMMARY_MODE_CARDINAL],
        default=SUMMARY_MODE_SENSOR,
        help="Summary columns: sensor (S1L..S4R) or cardinal (EBL..SBR). Default: sensor.",
    )
    args = ap.parse_args(argv[1:])

    if not args.bindata_txt:
        ap.print_help(sys.stderr)
        return 2

    src = Path(args.bindata_txt)
    if not src.is_file():
        print(f"Not found: {src}", file=sys.stderr)
        return 1
    out = Path(args.output_base) if args.output_base else src.with_name(src.stem + "_clean")
    ...
    summary_p, detail_p, diag, summ = write_outputs(
        ...
        sensor_to_cardinal=default_cardinals,
    )
    print(f"Wrote {summary_p} ({args.summary_mode} summary, {len(summ)} rows incl. Total if any)")
