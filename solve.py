        ...
        while end_date != start_date and nextstep == "yes":
            ...
            try:
                ...
                while camera_number == 1 and nextstep == "yes":
                    ...
                    # Ensure we have exactly 12 columns - use reindex to pad or truncate
                    expected_cols = 12
                    column_names = ["Zone ID", "Zone Name", "Time", "Average Speed", "Volume", "Occupancy", 
                                    "Through", "Right", "Left", "LefttoRight", "RighttoLeft"]
                    
                    # Reindex columns to ensure exactly expected_cols columns
                    if df1_split.shape[1] != expected_cols:
                        df1_split = df1_split.reindex(columns=range(expected_cols), fill_value=None)
            ...
            try:
                while camera_number < 5 and nextstep == "yes":
                    ...
                    # Reindex columns to ensure exactly expected_cols columns
                    if df2_split.shape[1] != expected_cols:
                        df2_split = df2_split.reindex(columns=range(expected_cols), fill_value=None)
